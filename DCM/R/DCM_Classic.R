DCM_Classic <-
function(M1, M2, max.groups = 5, max.iter = 50, max.time = 1, est.size = 100, alpha = 0.05, start = c(), QN = FALSE, resid.full = FALSE, echo = FALSE, strict = "low"){
	
	print("Version with LRT")
	...
	while(length(startdels) < p-kval & ans <= max.groups & tottime < max.time){
		...
		DCM = run_DCM_Classic(M1, M2, seed = tmp, echo = echo, max.iter = max.iter)
	...
	} # while(length(dels) < p1-kval & ans < max.groups)
	
	# Print reason for algorithm halting
	if(echo){
		if(tottime >= max.time){
			print("Timed out.")
		}else if(length(startdels) > p1-kval){
